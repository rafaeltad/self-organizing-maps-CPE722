...
class SOMVisualizer:
    ...
    def __init__(self, analyzer: TwitterSOMAnalyzer):
        ...
        if not analyzer.is_trained:
            warnings.warn(
                "Analyzer is not trained. Some visualizations may not work."
            )
    ...
    def plot_som_topology(
        ...
        """
        Plot the SOM topology with cluster assignments and log to MLflow.
        ...
        # Log plot to MLflow
        mlflow.log_figure(fig, "som_topology.png")
        plt.close(fig)
            ...
            for j in range(som._weights.shape[1]):
                ...
                for di in [-1, 0, 1]:
                    for dj in [-1, 0, 1]:
                        ni, nj = i + di, j + dj
                        if (
                            0 <= ni < som._weights.shape[0]
                            and 0 <= nj < som._weights.shape[1]
                            and (di != 0 or dj != 0)
                        ):
                ...
                if neighbors:
                    distances = [
                        np.linalg.norm(som._weights[i, j] - neighbor)
    ...
    def _plot_cluster_sizes(self, ax: plt.Axes) -> None:
        ...
        cluster_sizes = [
            stats["size"] for stats in self.analyzer.cluster_stats.values()
        ]
    ...
    def _plot_feature_map(self, ax: plt.Axes) -> None:
        ...
        plt.colorbar(
            im,
            ax=ax,
            label=f"Weight ({self.analyzer.feature_names[feature_idx]})",
        )
    ...
    def plot_cluster_analysis(
        ...
        """
        Plot comprehensive cluster analysis and log to MLflow.
        ...
        # Log plot to MLflow
        mlflow.log_figure(fig, "cluster_analysis.png")
        ...
        for stats in self.analyzer.cluster_stats.values():
            for hashtag, count in stats["dominant_hashtags"]:
                hashtag_counts[hashtag] = (
                    hashtag_counts.get(hashtag, 0) + count
                )
    ...
    def _plot_feature_importance_heatmap(self, ax: plt.Axes) -> None:
        ...
        for cluster_id in list(self.analyzer.cluster_stats.keys())[
            :10
        ]:  # Limit to 10 clusters
            importance = self.analyzer._get_cluster_feature_importance(
                cluster_id
            )
            top_features = sorted(
    ...
    def _plot_cluster_similarity(self, ax: plt.Axes) -> None:
        ...
        for i, cluster_id in enumerate(cluster_ids):
            similarities = self.analyzer._calculate_cluster_similarities(
                cluster_id
            )
    ...
    def create_interactive_visualization(self) -> None:
        """
        Create interactive Plotly visualization and log to MLflow.
        """
        ...
        if not self.analyzer.is_trained:
            raise ValueError(
                "Analyzer must be trained before creating visualizations"
            )
        ...
        temp_file = "temp_interactive_viz.html"
        fig.write_html(temp_file)
        mlflow.log_artifact(temp_file, artifact_path="visualizations")
        import os

        os.remove(temp_file)

    def _add_som_topology_plotly(
        self, fig: go.Figure, row: int, col: int
    ) -> None:
    ...
    def _add_engagement_scatter_plotly(
        self, fig: go.Figure, row: int, col: int
    ) -> None:
    ...
    def _add_temporal_bar_plotly(
        ...
        fig.add_trace(
            go.Bar(x=hours, y=counts, showlegend=False), row=row, col=col
        )

    def _add_feature_importance_plotly(
        self, fig: go.Figure, row: int, col: int
    ) -> None:
        ...
        if feature_matrix:
            fig.add_trace(
                go.Heatmap(
                    z=feature_matrix, colorscale="Viridis", showscale=False
                ),
                row=row,
                col=col,
            )

    def plot_tweet_distribution(
        self,
        figsize: Tuple[int, int] = (12, 8),
    ) -> None:
        """
        Plot tweet distribution across clusters with sample tweets and log to MLflow.
        ...
        cluster_ids = list(self.analyzer.cluster_stats.keys())
        sizes = [
            self.analyzer.cluster_stats[cid]["size"] for cid in cluster_ids
        ]
        ...
        ax1.set_xticklabels(
            [cid.replace("cluster_", "") for cid in cluster_ids], rotation=45
        )

        # Plot 2: Sample tweets from largest clusters
        # Get top 5 clusters by size
        top_clusters = sorted(
            self.analyzer.cluster_stats.items(),
        ...
        for cluster_id, stats in top_clusters:
            ...
            for i, tweet_text in enumerate(
                stats["sample_tweets"][:2]
            ):  # Show 2 sample tweets
        ...
        # Log plot to MLflow
        mlflow.log_figure(fig, "tweet_distribution.png")
        plt.close(fig)  # Close figure to free memory

    def generate_report(self) -> None:
        """
        Generate a text report of the SOM analysis and log to MLflow.
        """
        if not self.analyzer.is_trained:
            raise ValueError(
                "Analyzer must be trained before generating report"
            )
        ...
        with open(temp_file, "w") as f:
            ...
            for (
                cluster_id,
                cluster_stats,
            ) in self.analyzer.cluster_stats.items():
                f.write(f"\n{cluster_id.upper()}\n")
                f.write(f"Size: {cluster_stats['size']} tweets\n")
                f.write(
                    f"Average engagement: {cluster_stats['avg_engagement']:.2f}\n"
                )
                f.write(
                    f"Dominant hashtags: {cluster_stats['dominant_hashtags'][:3]}\n"
                )
                f.write(
                    f"Peak hour: {cluster_stats['time_distribution']['peak_hour']}\n"
                )
            ...
            f.write(
                f"Peak hours: {summary['temporal_overview']['peak_hours'][:3]}\n"
            )

        # Log report to MLflow
        mlflow.log_artifact(temp_file, artifact_path="reports")
        import os
